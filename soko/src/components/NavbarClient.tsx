...
// Dynamic import kwa LanguageToggle (sio muhimu mara moja)
...
export default function NavbarClient({
  ...
  return (
    <header className="bg-gradient-to-r from-night via-[#3B0A6B] to-market-600 text-market-50 sticky top-0 z-40 relative overflow-hidden">
      {/* Punguza SVG complexity kwa mobile */}
      <svg
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          {isLoggedIn ? (
            <>
              ...
              <Link href="/account" className="hidden sm:inline text-market-50/70 hover:text-market-50 transition-colors">
                {t.nav.hi}, {name?.split(' ')[0]}
              </Link>
              <button
                onClick={() => signOut({ redirectTo: '/' })}
                className="hidden sm:inline-flex btn btn-outline !border-market-50/30 !text-market-50 hover:bg-white/10 transition-colors"
              >
                {t.nav.signOut}
              </button>
            </>
            ...
            <>
              ...
              <Link href="/register" className="hidden sm:inline-flex btn btn-secondary hover:brightness-110 transition-colors">
                {t.nav.signup}
              </Link>
            </>
          )}
